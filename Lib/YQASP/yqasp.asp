...
Dim YQasp : Set YQasp = New YunQiAsp '实例化YQasp
%>
...
Class YunQiAsp
    ...
    Select Case t
      Case "on"
        For i = 0 To Ubound(a_core)
          Execute "Set " & a_core(i) & " = New YunQiAsp_object"
        Next
      Case "off"
  ...
  Public Function GetMethod()
    GetMethod = Request.ServerVariables("Request_Method")
  End Function
  '获取Header请求头指定信息
  Public Function GetHeaderVal(s)
    Dim raw : raw = Request.ServerVariables("All_Raw")
    Dim r : r = ""

    dim x : x=split(raw,chr(10))
    for i=0 to ubound(x)
      if IsN(x(i)) then exit for
      if LCase(trim(s)) = LCase(trim(YQAsp.Str.GetColonName(x(i)))) then
        r = trim(YQAsp.Str.Replace(YQAsp.Str.GetColonValue(x(i)), chr(13), ""))
        exit for
      end if
      'response.write YQAsp.Str.GetColonName(x(i))&"===="&YQAsp.Str.GetColonValue(x(i))&"<br>"
    next 
...
End Class
Class YunQiAsp_object : End Class
%>
